...
from ..tools.sms_api import SMSApi as SmsApi
...
class SmsSms(models.Model):
    ...
    def _send(self, unlink_failed=False, unlink_sent=True, raise_exception=False):
        ...
        try:
            results = SmsApi(self.env)._send_sms_batch(
        ...
        for iap_state, results_group in tools.groupby(results, key=lambda result: result['state']):
            sms_sudo = all_sms_sudo.filtered(
                lambda s: s.uuid in {result['uuid'] for result in results_group})
            if success_state := self.IAP_TO_SMS_STATE_SUCCESS.get(iap_state):
                ...
                failure_type = self.IAP_TO_SMS_FAILURE_TYPE.get(
                    iap_state, 'unknown')
