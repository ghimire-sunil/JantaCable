...
#     filter_brand_ids = fields.Many2many('product.brand', string='Brands')
    ...
    def onchange_warehouse(self):
        ...
        if total_warehouses:
            ...
            total_warehouses = self.env['stock.warehouse'].search(
                [('company_id', '=', self.company_id.id)])
            addtional_ids = []
            for warehouse in total_warehouses:
                store_location_id = warehouse.view_location_id.id
    ...
    def print_report(self):
        ...
        datas = {
            'form':
            {
                ...
                'id': self.id,
                'filter_product_ids': [p.id for p in self.filter_product_ids] or None,
                'filter_product_categ_ids': [p.id for p in self.filter_product_categ_ids] or None,
                # 'filter_brand_ids': [p.id for p in self.filter_brand_ids]
            }
        }
        ...
        if [y.id for y in self.warehouse_ids] and (not self.company_id):
            self.warehouse_ids = []
            raise UserError(
                _('Please select company of those warehouses to get correct view.\nYou should remove all warehouses first from selection field.'))
    ...
    def _product_detail(self, product_id):
        ...
        return product_name, product.barcode, product.default_code, product.standard_price
    ...
    def print_xls_report(self):
        ...
        datas = {
            ...
            {
                'company_id': self.company_id and [self.company_id.id] or [],
                'warehouse_ids': [y.id for y in self.warehouse_ids] or None,
                'location_id': self.location_id and self.location_id.id or False,
                # 'inventory_date': self.inventory_date,
                'start_date': self.start_date.strftime(DF),
                'end_date': self.end_date.strftime(DF),
        ...
        for company in company_ids:
            ...
            sheet_start_value = 4
            sheet.write_merge(sheet_start_header,
                              sheet_start_header, 0, 1, 'Date', header_tstyle_c)
            sheet.write_merge(sheet_start_value, sheet_start_value, 0, 1, self.start_date.strftime(
                DF) + ' To ' + self.end_date.strftime(DF), other_tstyle_cr)
            sheet.write_merge(sheet_start_header, sheet_start_header,
                              2, 3, 'Company', header_tstyle_c)
            sheet.write_merge(sheet_start_value, sheet_start_value,
                              2, 3, c_rec.name, other_tstyle_c)
            sheet.write_merge(sheet_start_header, sheet_start_header,
                              4, 5, 'Warehouse(s)', header_tstyle_c)
            sheet.write_merge(sheet_start_value, sheet_start_value,
                              4, 5, warehouses, other_tstyle_c)
            sheet.write_merge(sheet_start_header, sheet_start_header,
                              6, 7, 'Currency', header_tstyle_c)
            sheet.write_merge(sheet_start_value, sheet_start_value,
                              6, 7, c_rec.currency_id.name, other_tstyle_c)
            sheet.write_merge(sheet_start_header, sheet_start_header,
                              8, 9, 'Display', header_tstyle_c)
            sheet.write_merge(sheet_start_value,
                              sheet_start_value, 8, 9, summary, other_tstyle_c)

            if self.only_summary:
                header_row_start = 8
                sheet.col(0).width = 256 * 25
                sheet.write(header_row_start, 0,
                            'Category Name ', header_tstyle_c)
                sheet.col(1).width = 256 * 25
                sheet.write(header_row_start, 1,
                            'Total Beginning', header_tstyle_c)
                sheet.col(2).width = 256 * 25
                sheet.write(header_row_start, 2,
                            'Total Received', header_tstyle_c)
                sheet.col(3).width = 256 * 25
                sheet.write(header_row_start, 3,
                            'Total Sales', header_tstyle_c)
                sheet.col(4).width = 256 * 25
                sheet.write(header_row_start, 4,
                            'Total Adjustment', header_tstyle_c)
                sheet.col(5).width = 256 * 25
                sheet.write(header_row_start, 5,
                            'Total Ending', header_tstyle_c)
                sheet.col(6).width = 256 * 25
                sheet.write(header_row_start, 6,
                            'Total Values', header_tstyle_c)
                row = 9
                ...
                for key, values in total_lines.items():
                    ...
                    for line in values:
                        ...
                        product_qty_out = line.get(
                            'product_qty_out', 0.0) or 0.0
                        product_qty_adjustment = line.get(
                            'product_qty_adjustment', 0.0) or 0.0
                        beginning_inventory = line.get(
                            'begining_qty', 0.0) or 0.0
...
#                           )
                ...
                sheet.write(header_row_start, 1,
                            'Product Name ', header_tstyle_c)
                sheet.col(1).width = 256 * 40
                sheet.write(header_row_start, 2,
                            'Product Barcode ', header_tstyle_c)
                sheet.col(2).width = 256 * 20
                sheet.write(header_row_start, 3,
                            'Default Code ', header_tstyle_c)
                sheet.col(3).width = 256 * 20
                sheet.write(header_row_start, 4, 'Beginning', header_tstyle_c)

                sheet.write(header_row_start, 5, 'Beginning Valuation', header_tstyle_c)

                sheet.col(4).width = 256 * 20
                sheet.write(header_row_start, 6, 'Received', header_tstyle_c)

                sheet.write(header_row_start, 7, 'Received Valuation', header_tstyle_c)

                sheet.col(5).width = 256 * 20
                sheet.write(header_row_start, 8, 'Sales', header_tstyle_c)

                sheet.write(header_row_start, 9, 'Sales Valuation', header_tstyle_c)

                sheet.col(7).width = 256 * 20
                sheet.write(header_row_start, 10,
                            'Adjustments', header_tstyle_c)
                
                sheet.write(header_row_start, 11, 'Adjustments Valuation', header_tstyle_c)

                sheet.col(8).width = 256 * 20
                sheet.write(header_row_start, 12, 'Ending', header_tstyle_c)
                sheet.col(9).width = 256 * 20
                sheet.write(header_row_start, 13, 'Cost', header_tstyle_c)
                sheet.col(10).width = 256 * 20
                sheet.write(header_row_start, 14,
                            'Total Value', header_tstyle_c)
                sheet.col(11).width = 256 * 20
                row = 9
                grand_total_qty_in, grand_total_qty_out, \
                    grand_total_product_qty_adjustment, \
                    grand_total_beginning_inventory, \
                    grand_total_ending_inventory, grand_total_subtotal_cost,beginning_valuation_grand_total, received_grand_total_valuation, sales_grand_total_valuation, adjustment_grand_total_valuation = 0.0, \
...
#                           )
                        ...
                        product_name, product_barcode, product_code, prod_cp = \
                        ...
                        beginning_valuation_grand_total += beginning_inventory * prod_cp
                        received_grand_total_valuation += product_qty_in * prod_cp
                        sales_grand_total_valuation += product_qty_out * prod_cp
                        adjustment_grand_total_valuation += product_qty_adjustment * prod_cp
                        if self._value_existed(
                            ...
                            sheet.write(
                                row,
                                5,
                                "%.2f" % (beginning_inventory * prod_cp) ,
                                other_tstyle_r)
                            ...
                            sheet.write(
                                row,
                                11,
                                "%.2f" % (product_qty_adjustment * prod_cp),
                                other_tstyle_r)
                ...
                sheet.write(
                    row,
                    5,
                    "%.2f"% beginning_valuation_grand_total,
                    other_tstyle_grandr)
                sheet.write(
                    row,
                    6,
                ...
                sheet.write(
                    row,
                    7,
                    "%.2f"% received_grand_total_valuation,
                    other_tstyle_grandr)
                sheet.write(
                    row,
                    8,
                ...
                sheet.write(
                    row,
                    9,
                    "%.2f"% sales_grand_total_valuation,
                    other_tstyle_grandr)
                sheet.write(
                    row,
                    10,
                ...
                sheet.write(
                    row,
                    11,
                    "%.2f"% adjustment_grand_total_valuation,
                    other_tstyle_grandr)
                sheet.write(
                    row,
                    12,
