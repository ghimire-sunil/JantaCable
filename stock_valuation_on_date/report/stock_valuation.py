...
class StockValuationCategory(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        if not data.get('form'):
            raise UserWarning(_("Form content is missing, this report cannot be printed."))
        return {
            'doc_ids': self._ids,
            'docs': self,
            'data': data,
    ...
    def _get_cp(self,product_id):
        product_data = self.env['product.product'].search([('id','=',product_id),('company_id','=',self.env.company.id)])
        return product_data.standard_price
    ...
    def get_warehouse_name(self, warehouse_ids):
        ...
        if not warehouse_ids:
            warehouse_ids = [x.id for x in warehouse_obj.search([('company_id', '=', self.env.company.id)])]
...
#         stock_value = to_check.with_context(to_date=inventory_date, force_company=company_id).value_svl
        ...
        # return to_check.sudo().standard_price
    ...
    def category_wise_value(self, start_date, end_date, locations, company_id, filter_product_categ_ids=[]):
        """
        ...
        self._cr.execute('''
                        SELECT pp.id AS product_id, pt.categ_id,
                            0.0 AS begining_qty,
                            sum((
                                CASE WHEN sourcel.usage = 'internal' AND smline.location_id in %s  AND destl.usage !='inventory' AND sp.sale_id is not null
                                THEN -(smline.quantity)
                                ELSE 0.0 
                                END
                            )) AS product_qty_out,
                            sum((
                                CASE WHEN destl.usage = 'internal' AND smline.location_dest_id in %s AND sourcel.usage !='inventory' AND spt.code != 'internal'
                                THEN (smline.quantity)
                                ELSE 0.0 
                                END
                            )) AS product_qty_in,
                        ...
                        FROM product_product pp
                            ...
                            LEFT JOIN stock_picking sp ON (smline.picking_id = sp.id)
                            LEFT JOIN stock_picking_type spt ON (sp.picking_type_id = spt.id)
                        WHERE pt.company_id IS NULL OR pt.company_id = %s
                        GROUP BY pt.categ_id, pp.id ORDER BY pt.categ_id
                        ''',(tuple(locations),tuple(locations),tuple(locations),tuple(locations),start_date, end_date, company_id))
        ...
        self._cr.execute(''' 
                            ...
                            FROM
                                ...
                                SELECT
                                    ...
                                    coalesce(sum(-smline.quantity)::decimal, 0.0) as begining_qty
                                FROM stock_move_line smline 
                                LEFT JOIN product_product pp ON smline.product_id = pp.id
                                LEFT JOIN product_template pt ON pp.product_tmpl_id = pt.id
                                ...
                                SELECT
                                    ...
                                    coalesce(sum(smline.quantity)::decimal, 0.0) as begining_qty
                                FROM stock_move_line smline 
                                JOIN product_product pp ON smline.product_id = pp.id
                                ...
                                AS foo
                            GROUP BY product_id
                            ''',(start_date, tuple(locations), company_id, start_date, tuple(locations), company_id))
    ...
    def _get_categ(self, categ):
        ...
        if not self.env['product.category'].browse(categ).read(['name']):
    ...
    def _get_lines(self, data, company):
        ...
        records = self.category_wise_value(start_date, end_date, locations, company, filter_product_categ_ids)

        # records by categoriesNone
        _logger.info("Records: %s", records)   
        for record in records:
            if record.get('categ_id') is None:
                record['categ_id'] = 0
