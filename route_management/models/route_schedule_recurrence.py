...
class RouteScheduleRecurrence(models.Model):
    _name = 'route.schedule.recurrence'
    _description = 'Route Schedule Recurrence'

    route_ids = fields.One2many('user.route.schedule', 'recurrence_id', copy=False)
    ...
    def _get_last_route_id_per_recurrence_id(self):
        return {} if not self else {
            recurrence.id: max_route_schedule_id
            for recurrence, max_route_schedule_id in self.env['user.route.schedule'].sudo()._read_group(
    ...
    def _create_next_occurrence(self, occurrence_from):
        ...
        if (
            ...
            occurrence_from.with_context().sudo().copy(
    ...
    def _create_next_occurrence_values(self, occurrence_from):
        ...
        create_values['stage_id'] = occurrence_from.route_id.type_ids[0].id if occurrence_from.route_id.type_ids else occurrence_from.stage_id.id
        create_values['child_ids'] = [
            child.with_context().sudo().copy(self._create_next_occurrence_values(child)).id for child in occurrence_from.with_context(active_test=False).child_ids
        ]
